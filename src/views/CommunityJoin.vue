            ...
            <v-row dense>
              ...
              <v-col cols="12" md="3">
                <v-select
                  ...
                  label="縣市"
                  density="comfortable"
                  @update:modelValue="createForm.district = ''"
                />
              </v-col>
              ...
              <v-col cols="12" md="6">
                <v-text-field
                  ...
                  label="詳細地址"
                  density="comfortable"
                  :disabled="!createForm.city || !createForm.district"
                />
              </v-col>
...
// 台灣縣市清單
const cities = taiwanDistricts

// 動態取得選取縣市的區域
const districtsForSelectedCity = computed(() => {
  const selectedCity = taiwanDistricts.find((item) => item.city === createForm.value.city)
...
async function createCommunity() {
  ...
  try {
    const address = `${createForm.value.city}${createForm.value.district}${createForm.value.detailAddress}`
    const payload = {
      name: createForm.value.name,
      address,
      isPublic: createForm.value.isPublic,
    }
    const { data } = await api.post('/communities/create', payload)
